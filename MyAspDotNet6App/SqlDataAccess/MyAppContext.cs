...
public class MyAppContext
{
    ...
    private List<Dictionary<string, string?>> GetRowList(SqlCommand cmd, out DataTable schemaTable)
    {
    ...
    public byte[] CreateExcel(SqlCommand cmd, string sheetName)
    {
        byte[] bytes;
    ...
    private void OutputExcelSheet(ExcelPackage package, SqlCommand cmd, string sheetName)
    {
        ...
        try
        {
            rowList = GetRowList(cmd, out schemaTable);
        }
        catch (Exception ex)
        {
